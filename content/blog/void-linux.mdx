---
title: "Exploring Void Linux"
subtitle: "Minimal, Practical, and (Mostly) Painless"
date: "2025-05-14"
description: "My thoughts on Void Linux and some tips on using it."
tags: ["linux", "distro-talk", "minimalism"]
---
...
`xbps` is an amazing package manager with weird syntax. First, separate binaries is actually interesting. Keeping your install/query/remove separate makes for a more efficient way of updating, but the naming scheme, really guys? The first thing I recommend doing is going to your `.zshrc` or your `.bashrc` (or whatever hipster shell you use) and alias `xi` to `xbps-install`, `xr` to `xbps-remove`, `xq` to `xbps-query`, and `update` to `xbps-install -Su`. Now that you don't have to write a sentence to install a package, you might notice the only other two real "downsides." `xbps` is case-sensitive (so is `pacman` btw!)… but most packages in Arch are lowercase, unlike Void. Especially in the beginning, you might have issues finding the package you want. The Void Wiki was retired :(, so there are currently three methods to find things:

- Use the Void Wiki (most things are still relevant)
- Use the [official search page](https://voidlinux.org/packages/) (not a fan of unnecessarily opening my browser, but this is a good system, to be fair)
...
If you'd like to see some more articles about Void Linux, or have questions, feel free to shoot me an email at danielrose@member.fsf.org or Twitter [@TheCatster14](https://twitter.com/thecatster14).
